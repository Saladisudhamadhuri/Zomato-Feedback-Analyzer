...
if os.path.exists(CSV_FILE):
    ...
    saved_reviews = pd.read_csv(CSV_FILE)

    if 'db' not in st.session_state:
        st.session_state.db = saved_reviews.to_dict("records")
...
if option == "Feedback":
    ...
    for i, (name, info) in enumerate(st.session_state.p.items()):
        ...
        with cols[i]:
            ...
            with st.expander("View Reviews"):
                ...
                product_reviews = [
                    r for r in st.session_state.db
                    if r['prod'] == name
                ]
    ...
    with c1:
        ...
        pr = st.selectbox(
            "Which item did you try?",
            ["--select--"] + list(st.session_state.p.keys())
        )
    ...
    if not st.session_state.db:
        ...
        with c1:
            ...
            fig1 = px.histogram(
                ...
                x="prod",
                color="sent",
                title="Sentiment Distribution by Item",
                category_orders={
                    "sent": [
                        "Positive☺️",
                        "Neutral🫠",
        ...
        with c2:
            ...
            fig2 = px.area(
                df.groupby("prod")["rating"]
                .mean()
                .reset_index(),

                x="prod",
                y="rating",
                title="Average Rating by Item"
            )
        ...
        for _, row in df.iterrows():
            ...
            st.markdown(f"""
            <div class="review-box"
            style="border-color:{row['color']}20;
